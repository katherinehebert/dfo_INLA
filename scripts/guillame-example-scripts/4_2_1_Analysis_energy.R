...
for(j in 1:nyear){
  ...
  if(sum(spYear)>0){
    ...
    if(WAIC$fishInver[i] == "fish"){
      ...
      stackEst <- inla.stack(data = list(y = spYear),
                             ...
                             effect = list(s = 1:meshSpace$n,
                                           ...
                                           bottom.temperature = scale(explan@data$bottom.temperature)[pointerYear],
                                           surface.temperature = scale(explan@data$surface.temperature)[pointerYear]),
                             tag = "est")
      ...
      stackPred <- inla.stack(data = list(y = NA),
                              ...
                              effect = list(s = 1:meshSpace$n,
                                            ...
                                            bottom.temperature = scale(explanMesh@data$bottom.temperature)[pointerExplanMesh],
                                            surface.temperature = scale(explanMesh@data$surface.temperature)[pointerExplanMesh]),
                              tag = "pred")
      
      # Formula
      Formula <- y ~ 0 + intercept + bottom.temperature + surface.temperature + f(s, model = barrierModel)
    }
    ...
    if(WAIC$fishInver[i] == "invertebrate"){
      ...
      # Formula
      Formula <- y ~ 0 + intercept + bottom.temperature + f(s, model = barrierModel)
    }
