  ...
  if(sum(spYear)>0){
    ...
    stackEst <- inla.stack(data = list(y = spYear),
                           ...
                           effect = list(s = 1:meshSpace$n,
                                         ...
                                         slope = scale(explan@data$slope)[pointerYear],
                                         coarsness = scale(explan@data$coarsness)[pointerYear],
                                         bottom.salinity = scale(explan@data$bottom.salinity)[pointerYear]),
                           tag = "est")
    ...
    stackPred <- inla.stack(data = list(y = NA),
                            ...
                            effect = list(s = 1:meshSpace$n,
                                          ...
                                          slope = scale(explanMesh@data$slope)[pointerExplanMesh],
                                          coarsness = scale(explanMesh@data$coarsness)[pointerExplanMesh],
                                          bottom.salinity = scale(explanMesh@data$bottom.salinity)[pointerExplanMesh]),
                            tag = "pred")

    # Formula
    Formula <- y ~ 0 + intercept + slope + coarsness + bottom.salinity + f(s, model = barrierModel)
