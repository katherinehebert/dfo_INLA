...
summary(model_redfish_btbdp_rand_rand)

ExpY <- model_redfish_btbdp_rand_rand$summary.fitted.values[1:nrow(number@data),"mean"]

#For the variance we also need the posterior mean value of θ, which is obtained via
phi1 <- model_redfish_btbdp_rand_rand$summary.hyper[1, "mean"]
...
plot(x = model_redfish_btbdp_rand_rand$summary.fitted.values$mean[1:nrow(number@data)], 
     y = E1,
     xlab = "Fitted values",
...
for(i in 1:timeGr){
  ...
  fitMean = inla.mesh.project(mapBasis, 
                              model_redfish_btbdp_rand$summary.random$field$mean[1:SPDE$n.spde + (i - 1) * SPDE$n.spde])
  fit.025 = inla.mesh.project(mapBasis, 
                              model_redfish_btbdp_rand$summary.random$field$`0.025quant`[1:SPDE$n.spde + (i - 1) * SPDE$n.spde])
  fit.975 = inla.mesh.project(mapBasis, 
                              model_redfish_btbdp_rand$summary.random$field$`0.975quant`[1:SPDE$n.spde + (i - 1) * SPDE$n.spde])
