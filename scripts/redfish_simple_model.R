...
form_redfish_simple = redfish ~ 0 + bTemp + 
  f(field, model=SPDE,
    group = field.group,
    control.group=list(model='ar1', hyper=hSpec))


model_redfish_simple_rand_rand = inla(form_redfish_simple,
...
summary(model_redfish_simple_rand_rand)
saveRDS(model_redfish_simple_rand_rand, 
        here('./model-output/model_redfish_simple_rand_space.rds'))
